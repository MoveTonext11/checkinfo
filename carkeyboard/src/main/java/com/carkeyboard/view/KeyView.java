...
import com.carkeyboard.support.DrawableTint;
...
class KeyView extends TextView {
    ...
    public void bindKey(KeyEntry bindKey) {
        ...
        if (bindKey.keyType == KeyType.OK) {
            final Drawable drawable = ContextCompat.getDrawable(getContext(), R.drawable
                    .pwk_keyboard_key_general_bg);
            final ColorStateList tintColor = ContextCompat.getColorStateList(getContext(), R
                    .color.pwk_keyboard_key_ok_tint_color);
            final Drawable tintDrawable = DrawableTint.tint(drawable, tintColor);
            setBackgroundDrawable(tintDrawable);
            setTextColor(ContextCompat.getColorStateList(getContext(), R.color
                    .pwk_keyboard_key_ok_text));
        } else {
            setTextColor(ContextCompat.getColorStateList(getContext(), R.color
                    .pwk_keyboard_key_text));
    ...
    private void drawDeleteKey(Canvas canvas) {
        ...
        if (mDeleteDrawable == null) {
            ...
            mDeleteDrawable.setBounds(0, 0, mDeleteDrawable.getIntrinsicWidth() / 3 * 2,
                    mDeleteDrawable.getIntrinsicHeight() / 5 * 3);
        }
        canvas.save();
        canvas.translate((getWidth() - mDeleteDrawable.getIntrinsicWidth() / 4 * 3),
                mDeleteDrawable.getIntrinsicHeight() / 5);

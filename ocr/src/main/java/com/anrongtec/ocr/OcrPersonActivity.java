 ...
 * OCR 身份证扫描页面。
 */
public class OcrPersonActivity extends Activity implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    private void findView() {
        ...
        if (myView == null) {
            myView = new SIDViewfinderView(OcrPersonActivity.this, screenWidth, screenHeight);
            mainRl.addView(myView);
        }

        surfaceHolder = mSurfaceView.getHolder();
        surfaceHolder.addCallback(OcrPersonActivity.this);
        ...
        ibFlashOff.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_FLASH)) {
                    ...
                    Toast.makeText(OcrPersonActivity.this, mess, Toast.LENGTH_SHORT).show();
                } else {
                    if (mCamera != null) {
    ...
    private void initCardKernal() {
        if (!bInitKernal) {
            ...
            TelephonyManager telephonyManager = (TelephonyManager) getSystemService(Context
                    .TELEPHONY_SERVICE);
            int nRet = sidApi.SIDCardKernalInit("", useridPath, UserIdUtils.UserID, 0x02, 0x02,
                    telephonyManager, this);
            if (nRet != 0) {
                Toast.makeText(OcrPersonActivity.this, "激活失败", Toast.LENGTH_SHORT).show();
    ...
    private void initCamera() {
        ...
        Camera.Size tmpsize = getOptimalPreviewSize(list, screenWidth + navigationBarHeight,
                screenHeight);
        ...
        if (length == 1) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (size.height > 700 && size.height < previewheight) {
                    if (size.width * previewheight == size.height * previewWidth && size.height <
                            second_previewheight) {
        ...
        mCamera.setPreviewCallback(OcrPersonActivity.this);
        mCamera.setParameters(parameters);
        ...
        if (!alertDialog.isShowing()) {
            ...
            if (r == 0) {
                ...
                if (!TextUtils.isEmpty(sfzh)) {
                    //增加头像存储路径地址返回  Aton
                    intent.putExtra("headImage", strCaptureFileHeadPath);
